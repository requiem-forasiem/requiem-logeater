...
typedef struct {
        ...
        logeater_log_source_t *source;

        FILE *fd;
        FILE *metadata_fd;
        requiem_bool_t need_position;
...
static REQUIEM_LIST(active_fd_list);
static REQUIEM_LIST(inactive_fd_list);
...
static int stat_to_file(monitor_fd_t *fd, ev_statdata *st, idmef_target_t *target, idmef_file_category_t category)
{
        ...
        fname = logeater_log_source_get_name(fd->source);
        fptr = strrchr(fname, '/');
        requiem_string_set_ref(string, (fptr) ? (fptr + 1) : fname);

        ret = idmef_file_new_path(file, &string);
        if ( ret < 0 )
                return -1;
        requiem_string_set_ref(string, fname);
                          ...
                          idmef_classification_t *classification, idmef_impact_t *impact)
{
        ...
        logeater_alert_emit(fd->source, log_entry, message);
...
static void logfile_modified_alert(monitor_fd_t *monitor, ev_statdata *st_old, struct stat *st_new)
{
        ...
        requiem_string_set_constant(str, LOGFILE_MODIFICATION_CLASS);
        ...
        requiem_string_set_constant(str, LOGFILE_MODIFICATION_IMPACT);
...
static off_t read_logfile(monitor_fd_t *fd, off_t *available)
{
        ...
        while ( 1 ) {
                ...
                if ( fd->current_line_len <= LOG_LINE_MAXSIZE ) {
                        ret = requiem_string_ncat(fd->buf, (const char *) &ret, 1);
                        if ( ret < 0 )
                                requiem_log(REQUIEM_LOG_ERR, "error buffering input: %s.\n", requiem_strerror(ret));
                }

                else if ( fd->current_line_len == LOG_LINE_MAXSIZE + 1 )
                        requiem_log(REQUIEM_LOG_WARN, "line too long (internal limit of %u characters).\n", LOG_LINE_MAXSIZE);
...
static int file_metadata_read(monitor_fd_t *monitor, off_t *start, unsigned char **buf)
{
        ...
        if ( fstat(fileno(monitor->metadata_fd), &st) < 0 ) {
                requiem_log(REQUIEM_LOG_WARN, "fstat failed : %s.\n", strerror(errno));
                return -1;
        }
...
static int file_metadata_save(monitor_fd_t *monitor, off_t offset)
{
        ...
        gcry_md_hash_buffer(METADATA_CHECKSUM_TYPE, buf + sizeof(offset),
                            requiem_string_get_string(monitor->buf),
                            requiem_string_get_len(monitor->buf));

        rewind(monitor->metadata_fd);

        ret = fwrite(buf, 1, sizeof(buf), monitor->metadata_fd);
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "could not write metadata file: %s.\n", strerror(errno));
                return -1;
        }
...
static int verify_metadata_checksum(requiem_string_t *log, unsigned char *metadata_sum)
{
        unsigned char sum[METADATA_CHECKSUM_SIZE];

        assert(gcry_md_get_algo_dlen(METADATA_CHECKSUM_TYPE) == sizeof(sum));
        gcry_md_hash_buffer(METADATA_CHECKSUM_TYPE, sum, requiem_string_get_string(log), requiem_string_get_len(log));
...
static int file_metadata_get_position(monitor_fd_t *monitor)
{
        ...
        filename = logeater_log_source_get_name(monitor->source);
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "stat: error on file %s: %s.\n", filename, strerror(errno));
                return -1;
        }
        ...
        if ( ! have_metadata ) {
                requiem_log(REQUIEM_LOG_INFO, "%s: No metadata available, starting from tail.\n", filename);
                return fseeko(monitor->fd, st.st_size, SEEK_SET);
        }

        if ( st.st_size < offset ) {
                requiem_log(REQUIEM_LOG_INFO, "%s: log file was rotated, starting from head.\n", filename);
                monitor->last_size = 0;
                return 0;
        }
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "%s: error seeking to offset %" REQUIEM_PRIu64 ": %s.\n",
        ...
        if ( read_logfile(monitor, &available) < 0 || verify_metadata_checksum(monitor->buf, sumptr) != 0 ) {
                ...
                requiem_string_clear(monitor->buf);
        ...
        requiem_log(REQUIEM_LOG_INFO, "%s: resuming log analyzis at offset %" REQUIEM_PRIu64 ".\n",
...
static int file_metadata_open(monitor_fd_t *monitor)
{
        ...
        strncpy(file, logeater_log_source_get_name(monitor->source), sizeof(file));
        ...
        if ( fd < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "error creating %s: %s.\n", path, strerror(errno));
                return -1;
        }
        ...
        if ( ! monitor->metadata_fd ) {
                requiem_log(REQUIEM_LOG_ERR, "fdopen failed: %s.\n", strerror(errno));
                close(fd);
                return -1;
        }
...
static int check_logfile_data(monitor_fd_t *monitor, struct stat *st)
{
        ...
        while ( len && (ret = read_logfile(monitor, &len)) >= 0 ) {
                 ...
                 */
                if ( (slen = requiem_string_get_len(monitor->buf)) ) {
                        logeater_dispatch_log(monitor->source, requiem_string_get_string(monitor->buf), slen);
...
static int check_stdin_data(monitor_fd_t *monitor)
{
        ...
        if ( config.batch_mode )
                ...
                if ( ret < 0 ) {
                        requiem_log(REQUIEM_LOG_ERR, "FIONREAD failed on stdin: %s.\n", strerror(errno));
                        return -1;
                }
...
static monitor_fd_t *monitor_new(logeater_log_source_t *ls)
{
        int ret;
        monitor_fd_t *new;
        ...
        if ( strcmp(logeater_log_source_get_name(ls), STDIN_FILENAME) != 0 ) {
                ev_stat_init(&new->event.st, libev_stat_cb, logeater_log_source_get_name(new->source), 1);
        ...
        requiem_list_add(&inactive_fd_list, &new->list);
...
static int monitor_set_position(monitor_fd_t *monitor, const char *filename)
{
        ...
        if ( metadata_flags & FILE_SERVER_METADATA_FLAGS_LAST )
                ...
                requiem_log(REQUIEM_LOG_INFO, "%s: user requested starting from head.\n", filename);
                ret = fseeko(monitor->fd, 0, SEEK_SET);
        }

        else if ( metadata_flags & FILE_SERVER_METADATA_FLAGS_TAIL ) {
                requiem_log(REQUIEM_LOG_INFO, "%s: user requested starting from tail.\n", filename);
                ret = fseeko(monitor->fd, 0, SEEK_END);
        }

        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "%s: error seeking to the %s of the file: %s.\n",
...
{
        int ret, fd;
        const char *filename = logeater_log_source_get_name(monitor->source);

        ret = requiem_string_new(&monitor->buf);
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "could not create string object: %s.\n", requiem_strerror(ret));
                return ret;
        }
        ...
        if ( strcmp(filename, STDIN_FILENAME) == 0 )
                ...
                if ( ! monitor->fd ) {
                        if ( errno == ENOENT && monitor->prev_errno != errno )
                                requiem_log(REQUIEM_LOG_WARN, "%s does not exist.\n", filename);

#if !((defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__)
                        else if ( errno == EACCES && monitor->prev_errno != errno )
                                requiem_log(REQUIEM_LOG_WARN, "%s is not available for reading to uid %d/gid %d.\n",
                                            filename, getuid(), getgid());
#endif

                        monitor->prev_errno = errno;
                        requiem_string_destroy(monitor->buf);
                        return -1;
                }
                ...
                if ( ret == 0 )
                        requiem_log(REQUIEM_LOG_INFO, "%s: now available for monitoring.\n", filename);
        }

        monitor->need_more_read = 0;
        requiem_list_del(&monitor->list);
        requiem_list_add_tail(&active_fd_list, &monitor->list);
...
static void monitor_close(monitor_fd_t *monitor)
{
        ...
        requiem_list_del(&monitor->list);
        requiem_list_add_tail(&inactive_fd_list, &monitor->list);
}
...
static void check_modification_time(monitor_fd_t *monitor, ev_statdata *prev, struct stat *st)
{
        ...
        if ( st->st_size < monitor->last_size ) {
                ret = fseeko(monitor->fd, 0, SEEK_END);
                if ( ret < 0 ) {
                        requiem_log(REQUIEM_LOG_ERR, "%s: error seeking to end of file: %s.\n", filename, strerror(errno));
                        return;
                }
...
static int is_file_already_used(monitor_fd_t *monitor, ev_statdata *st_prev, ev_statdata *st_cur, struct stat *st_now)
{
        ...
        if ( st_now->st_nlink > 0 ) {
                requiem_log(REQUIEM_LOG_INFO, "%s: has been renamed.\n", filename);
                ctxt = LOGFILE_RENAME_CLASS;
                is_deleted = FALSE;
        } else {
                requiem_log(REQUIEM_LOG_INFO, "%s: has been deleted.\n", filename);
        ...
        requiem_string_set_ref(str, ctxt);
        ...
        if ( toff <= max_rotation_time_offset || soff <= max_rotation_size_offset ) {
                ...
                if ( ret < 0 )
                        return ret;
                requiem_string_set_constant(str, LOGFILE_DELETION_IMPACT);
        } else {
                idmef_impact_set_severity(impact, IDMEF_IMPACT_SEVERITY_HIGH);
                ret = idmef_impact_new_description(impact, &str);
                if ( ret < 0 )
                ...
                snprintf(buf, sizeof(buf), "An inconsistency has been observed in file rotation: "
                        ...
                        "than allowed limits: %" REQUIEM_PRIu64 " bytes difference (%" REQUIEM_PRIu64" allowed), "
                        "%" REQUIEM_PRIu64 " seconds difference (%" REQUIEM_PRIu64" allowed)",
                         (intmax_t) soff, (intmax_t) max_rotation_size_offset,
                         (intmax_t) toff, (intmax_t) max_rotation_time_offset);
                requiem_string_set_ref(str, buf);
        }
...
int file_server_monitor_file(logeater_log_source_t *ls)
{
        monitor_fd_t *new;
...
int file_server_read_once(void)
{
        ...
        requiem_list_for_each_safe(&active_fd_list, tmp, bkp) {
                monitor = requiem_list_entry(tmp, monitor_fd_t, list);
...
int file_server_start_monitoring(void)
{
        ...
        requiem_list_for_each_safe(&inactive_fd_list, tmp, bkp) {
                monitor = requiem_list_entry(tmp, monitor_fd_t, list);

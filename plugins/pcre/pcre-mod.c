...
int pcre_LTX_requiem_plugin_version(void);
int pcre_LTX_logeater_plugin_init(requiem_plugin_entry_t *pe, void *data);
...
struct pcre_plugin {
        ...
        requiem_bool_t dump_unmatched;

        requiem_list_t rule_list;
...
static int parse_ruleset(requiem_list_t *head, pcre_plugin_t *plugin, const char *filename, FILE *fd);




static REQUIEM_LIST(chained_rule_list);
static logeater_log_plugin_t pcre_plugin;
...
static int parse_rule_regex(pcre_plugin_t *plugin, pcre_rule_t *rule, const char *regex)
{
        ...
        requiem_linked_object_add_tail(&rule->regex_list, (requiem_linked_object_t *) new);
...
static pcre_rule_container_t *search_rule(requiem_list_t *head, unsigned int id)
{
        requiem_list_t *tmp;
        pcre_rule_container_t *cur;

        requiem_list_for_each(head, tmp) {
                cur = requiem_list_entry(tmp, pcre_rule_container_t, list);
...
static int add_goto_single(pcre_plugin_t *plugin, pcre_rule_t *rule, unsigned int id, requiem_bool_t optional)
{
        ...
        if ( ! cur ) {
                ...
                if ( ! cur ) {
                        requiem_log(REQUIEM_LOG_WARN, "could not find a rule with ID %d.\n", id);
                        return -1;
                }
        }
        ...
        requiem_list_add_tail(&rule->rule_list, &new->list);

        return 0;
}


static int add_goto(pcre_plugin_t *plugin, pcre_rule_t *rule, const char *idstr, requiem_bool_t optional)
{
        int ret;
        ...
        if ( ret < 1 ) {
                requiem_log(REQUIEM_LOG_WARN, "could not parse goto value '%s'.\n", idstr);
                return -1;
        }
...
static int add_value_to_list(requiem_list_t *head, const char *arg, void *data)
{
        int ret;
        ...
        requiem_linked_object_add_tail(head, (requiem_linked_object_t *) vcont);
...
static int _parse_create_context(pcre_rule_t *rule, const char *arg, pcre_context_setting_flags_t flags)
{
        ...
        while ( (ret = parse_multiple_key_and_value(&arg, &key, &value)) == 1 ) {
                ...
                if ( ! cname )
                        ...
                        free(pcs);
                        requiem_log(REQUIEM_LOG_WARN, "Unknown context creation argument: '%s'.\n", key);
                        return -1;
                }
        }
...
static int parse_not_context(pcre_plugin_t *plugin, pcre_rule_t *rule, const char *arg)
{
        ...
        requiem_linked_object_add_tail(&rule->not_context_list, (requiem_linked_object_t *) vcont);
...
static int parse_include(pcre_rule_t *rule, pcre_plugin_t *plugin, const char *value)
{
        ...
        fd = fopen(filename, "r");
        if ( ! fd ) {
                requiem_log(REQUIEM_LOG_ERR, "couldn't open %s for reading: %s.\n", filename, strerror(errno));
                return -1;
        }
...
static int parse_rule_included(pcre_plugin_t *plugin, pcre_rule_t *rule, const char *value)
{
        ...
        tmp.rule = rule;
        requiem_list_add(&plugin->rule_list, &tmp.list);

        ret = parse_include(rule, plugin, value);
        requiem_list_del(&tmp.list);

        requiem_list_for_each(&rule->rule_list, t) {
                cur = requiem_list_entry(t, pcre_rule_container_t, list);
                cur->optional = 1;
        }
                              ...
                              const char *keyword, const char *value)
{
        ...
        for ( i = 0; i < sizeof(keywords) / sizeof(keywords[0]); i++ ) {
                ...
                if ( keywords[i].func(plugin, rule, value) < 0 ) {
                        requiem_log(REQUIEM_LOG_WARN, "%s:%d: error parsing value for '%s'.\n", filename, line, keyword);
                        return -1;
                }
...
static pcre_rule_t *create_rule(void)
{
        ...
        requiem_list_init(&rule->rule_list);
        requiem_list_init(&rule->regex_list);
        requiem_list_init(&rule->not_context_list);
        requiem_list_init(&rule->create_context_list);
        requiem_list_init(&rule->destroy_context_list);
...
static void free_rule(pcre_rule_t *rule)
{
        ...
        requiem_list_for_each_safe(&rule->rule_list, tmp, bkp) {
                rc = requiem_list_entry(tmp, pcre_rule_container_t, list);
                free_rule_container(rc);
        }

        requiem_list_for_each_safe(&rule->regex_list, tmp, bkp) {
        ...
        requiem_list_for_each_safe(&rule->create_context_list, tmp, bkp)
                free_value(requiem_linked_object_get_object(tmp));

        requiem_list_for_each_safe(&rule->not_context_list, tmp, bkp)
                free_value(requiem_linked_object_get_object(tmp));

        requiem_list_for_each_safe(&rule->destroy_context_list, tmp, bkp)
                free_value(requiem_linked_object_get_object(tmp));
...
static void free_rule_container(pcre_rule_container_t *rc)
{
        ...
        requiem_list_del(&rc->list);
        free(rc);
}



static int parse_ruleset_directive(requiem_list_t *head, pcre_plugin_t *plugin, const char *filename, int line, char *buf)
{
        char *in;
        char *key;
        char *value;
        ...
        while ( (in = cut_line(buf, &ptr)) ) {
                ...
                if ( parse_key_and_value(in, &key, &value) < 0 ) {
                        requiem_log(REQUIEM_LOG_WARN, "%s:%d: no string delimiter.\n", filename, line);
                        return -1;
                }
        ...
        if ( requiem_list_is_empty(&rule->regex_list) ) {
                requiem_log(REQUIEM_LOG_WARN, "%s:%d: rule does not provide a regex.\n", filename, line);
                free_rule(rule);
                return -1;
        }
        ...
        if ( rule->flags & PCRE_RULE_FLAGS_CHAINED )
                requiem_list_add(&chained_rule_list, &rc->list);

        else if ( plugin->last_rules_first && rule->flags & PCRE_RULE_FLAGS_LAST )
                requiem_list_add(head, &rc->list);
        else
                requiem_list_add_tail(head, &rc->list);
...
static int parse_ruleset(requiem_list_t *head, pcre_plugin_t *plugin, const char *filename, FILE *fd)
{
        ...
        while ( requiem_read_multiline(fd, &line, buf, sizeof(buf)) == 0 ) {
...
static void pcre_run(requiem_plugin_instance_t *pi, const logeater_log_source_t *ls, logeater_log_entry_t *log_entry)
{
        int ret;
        requiem_list_t *tmp;
        ...
        requiem_log_debug(10, "\nInput = %s\n", logeater_log_entry_get_message(log_entry));

        plugin = requiem_plugin_instance_get_plugin_data(pi);

        requiem_list_for_each(&plugin->rule_list, tmp) {
                rc = requiem_list_entry(tmp, pcre_rule_container_t, list);
        ...
        if ( !(all_flags & PCRE_MATCH_FLAGS_ALERT) && plugin->dump_unmatched )
                requiem_log(REQUIEM_LOG_WARN, "No alert emited for log entry \"%s\"\n",
                            logeater_log_entry_get_message(log_entry));
}
...
static int set_last_first(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        pcre_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(context);
...
static int set_dump_unmatched(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        pcre_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(context);
...
static void remove_top_chained(void)
{
        ...
        requiem_list_for_each_safe(&chained_rule_list, tmp, bkp) {
                rc = requiem_list_entry(tmp, pcre_rule_container_t, list);
...
static int set_pcre_ruleset(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        int ret;
        FILE *fd;
        char *ptr;
        ...
        fd = fopen(optarg, "r");
        if ( ! fd ) {
                requiem_string_sprintf(err, "couldn't open %s for reading: %s", optarg, strerror(errno));
                return -1;
        }
        ...
        requiem_log(REQUIEM_LOG_INFO, "PCRE plugin loaded %d rules.\n", plugin->rulesnum);
...
static int pcre_activate(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context)
{
        pcre_plugin_t *new;
        ...
        requiem_list_init(&new->rule_list);
...
static void pcre_destroy(requiem_plugin_instance_t *pi, requiem_string_t *err)
{
        requiem_list_t *tmp, *bkp;
        pcre_rule_container_t *rule;
        pcre_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(pi);

        requiem_list_for_each_safe(&plugin->context_list, tmp, bkp)
                pcre_context_destroy(requiem_list_entry(tmp, pcre_context_t, list));

        requiem_list_for_each_safe(&plugin->rule_list, tmp, bkp) {
                rule = requiem_list_entry(tmp, pcre_rule_container_t, list);
...
{
        requiem_log_debug(1, "[%s]: destroying context.\n", ctx->name);
        ...
        requiem_timer_destroy(&ctx->timer);
        requiem_list_del(&ctx->list);
...
static void alert_from_context(pcre_state_t *state)
{
         ...
         */
        logeater_alert_set_infos(state->idmef, state->le);
        logeater_alert_emit(NULL, NULL, state->idmef);
}
...
void pcre_context_destroy(pcre_context_t *ctx)
{
        if ( ctx->setting->flags & PCRE_CONTEXT_SETTING_FLAGS_ALERT_ON_DESTROY && ctx->state->idmef ) {
                requiem_log_debug(1, "[%s]: emit alert on destroy.\n", ctx->name);
...
static void pcre_context_expire(void *data)
{
        ...
        if ( ctx->setting->flags & PCRE_CONTEXT_SETTING_FLAGS_ALERT_ON_EXPIRE && ctx->state->idmef ) {
                requiem_log_debug(1, "[%s]: emit alert on expire.\n", ctx->name);
                alert_from_context(ctx->state);
        }
...
int pcre_context_new(pcre_plugin_t *plugin, const char *name, pcre_state_t *state, pcre_context_setting_t *setting)
{
        ...
        if ( ! (setting->flags & PCRE_CONTEXT_SETTING_FLAGS_QUEUE) ) {
                ...
                if ( ctx ) {
                        if ( setting->flags & PCRE_CONTEXT_SETTING_FLAGS_OVERWRITE ) {
                                requiem_log_debug(1, "[%s]: destroying on create (overwrite).\n", name);
                                pcre_context_destroy(ctx);
                        } else {
                                requiem_log_debug(1, "[%s]: already exist, create only specified.\n", name);
                                return 0;
                        }
                }
        }

        requiem_log_debug(1, "[%s]: creating context (expire=%ds).\n", name, setting->timeout);
        ...
        setting->refcount += 1;
        ctx->setting = setting;
        requiem_timer_init_list(&ctx->timer);

        if ( setting->timeout > 0 ) {
                requiem_timer_set_data(&ctx->timer, ctx);
                requiem_timer_set_expire(&ctx->timer, setting->timeout);
                requiem_timer_set_callback(&ctx->timer, pcre_context_expire);
                requiem_timer_init(&ctx->timer);
        }

        requiem_list_add_tail(&plugin->context_list, &ctx->list);
...
pcre_context_t *pcre_context_search(pcre_plugin_t *plugin, const char *name)
{
        ...
        requiem_list_for_each(&plugin->context_list, tmp) {
                ctx = requiem_list_entry(tmp, pcre_context_t, list);
...
int pcre_LTX_logeater_plugin_init(requiem_plugin_entry_t *pe, void *logeater_root_optlist)
{
        int ret;
        requiem_option_t *opt, *popt;
        int hook = REQUIEM_OPTION_TYPE_CLI|REQUIEM_OPTION_TYPE_CFG;

        ret = requiem_option_add(logeater_root_optlist, &opt, hook, 0, "pcre", "Pcre plugin option",
                                 REQUIEM_OPTION_ARGUMENT_OPTIONAL, pcre_activate, NULL);
        if ( ret < 0 )
                return ret;

        requiem_plugin_set_activation_option(pe, opt, NULL);

        ret = requiem_option_add(opt, NULL, hook, 'r', "ruleset", "Ruleset to use",
                                 REQUIEM_OPTION_ARGUMENT_REQUIRED, set_pcre_ruleset, NULL);
        if ( ret < 0 )
                return ret;

        ret = requiem_option_add(opt, &popt, REQUIEM_OPTION_TYPE_CLI|REQUIEM_OPTION_TYPE_CFG, 'l',
                                 "last-first", "Process rules with the \"last\" attribute first",
                                 REQUIEM_OPTION_ARGUMENT_NONE, set_last_first, NULL);
        if ( ret < 0 )
                return ret;
        requiem_option_set_priority(popt, REQUIEM_OPTION_PRIORITY_FIRST);

        ret = requiem_option_add(opt, &popt, REQUIEM_OPTION_TYPE_CLI, 0,
                                 "dump-unmatched", "Dump unmatched log entry",
                                 REQUIEM_OPTION_ARGUMENT_NONE, set_dump_unmatched, NULL);
        if ( ret < 0 )
                return ret;

        pcre_plugin.run = pcre_run;
        requiem_plugin_set_name(&pcre_plugin, "pcre");
        requiem_plugin_set_destroy_func(&pcre_plugin, pcre_destroy);

        requiem_plugin_entry_set_plugin(pe, (void *) &pcre_plugin);

...
static int do_pcre_exec(rule_regex_t *item, const char *in, size_t len, int *omin, int *omax)
{
        ...
        if ( ret < -1 || ret == 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "unexpected PCRE error: %d.\n", ret);
                return -1;
        }
        ...
        *omax = optr[1];
        ...
        if ( ovector_index > OVECSIZE ) {
                requiem_log(REQUIEM_LOG_ERR, "backward references vector is too small: %u entry required.\n", ovector_index);
                return -1;
        }
...
static int exec_regex(pcre_rule_t *rule, const logeater_log_entry_t *log_entry)
{
        size_t len;
        rule_regex_t *item;
        requiem_list_t *tmp;
        ...
        len = logeater_log_entry_get_message_len(log_entry);
        subject = logeater_log_entry_get_message(log_entry);

        requiem_list_for_each(&rule->regex_list, tmp) {
                item = requiem_linked_object_get_object(tmp);

                ret = do_pcre_exec(item, subject, len, &omin, &omax);
                requiem_log_debug(5, "id=%d match=%s pcre_exec=%d\n", rule->id, item->regex_string, ret);
        ...
        if ( rule->min_optregex_match ) {
                requiem_log_debug(10, "optmatch=%d >= wanted=%d\n", optional_match, rule->min_optregex_match);
...
static pcre_context_t *lookup_context(value_container_t *vcont, pcre_plugin_t *plugin,
                                      pcre_rule_t *rule, const logeater_log_entry_t *log_entry)
{
        pcre_context_t *ctx;
        ...
        ctx = pcre_context_search(plugin, requiem_string_get_string(str));
...
static void pcre_state_init(pcre_state_t *state)
{
        ...
        requiem_list_init(&state->additional_data_list);
}
...
static int pcre_state_add_rule_infos(pcre_state_t *state, pcre_rule_t *rule, const logeater_log_source_t *ls, const logeater_log_entry_t *le)
{
        int ret;
        requiem_string_t *str;
        ...
        if ( ! state->le_added ) {
                state->le_added = TRUE;
                logeater_additional_data_prepare(&state->additional_data_list, ls, le);
        }
        ...
        if ( rule->id ) {
                ...
                requiem_string_set_constant(str, "Rule ID");
                idmef_additional_data_set_integer(ad, rule->id);

                requiem_linked_object_add_tail(&state->additional_data_list, (requiem_linked_object_t *) ad);
        }
        ...
        if ( rule->revision ) {
                ...
                requiem_string_set_constant(str, "Rule Revision");
...
int pcre_state_push_idmef(pcre_state_t *state, idmef_message_t *idmef)
{
        ...
        requiem_list_for_each_safe(&state->additional_data_list, tmp, bkp) {
                ad = requiem_linked_object_get_object(tmp);
                requiem_linked_object_del((requiem_linked_object_t *)ad);
...
{
        if ( state->le )
                logeater_log_entry_destroy(state->le);
...
int pcre_state_clone(pcre_state_t *state, pcre_state_t **new)
{
        ...
        requiem_list_for_each_safe(&state->additional_data_list, tmp, bkp) {
                ...
                requiem_linked_object_add_tail(&(*new)->additional_data_list, (requiem_linked_object_t *) ad);
        }

        if ( state->le )
                (*new)->le = logeater_log_entry_ref(state->le);

        return 0;
}


static int match_rule_single(pcre_plugin_t *plugin, pcre_rule_t *rule, pcre_state_t **state, const logeater_log_entry_t *log_entry)
{
        int ret;
        requiem_list_t *tmp;
        pcre_context_t *ctx;
        ...
        requiem_list_for_each(&rule->not_context_list, tmp) {
                vcont = requiem_linked_object_get_object(tmp);
...
static void create_context_if_needed(pcre_plugin_t *plugin, pcre_rule_t *rule, pcre_state_t *state, logeater_log_entry_t *log_entry)
{
        requiem_list_t *tmp;
        requiem_string_t *str;
        value_container_t *vcont;
        pcre_context_setting_t *pcs;
        logeater_log_entry_t *prev = state->le;

        state->le = log_entry;
        requiem_list_for_each(&rule->create_context_list, tmp) {
                vcont = requiem_linked_object_get_object(tmp);
                ...
                pcre_context_new(plugin, requiem_string_get_string(str), state, pcs);
                requiem_string_destroy(str);
        }
        state->le = prev;
}
...
static void destroy_context_if_needed(pcre_plugin_t *plugin, pcre_rule_t *rule,
                                      const logeater_log_entry_t *log_entry)
{
        pcre_context_t *ctx;
        requiem_list_t *tmp;
        requiem_string_t *str;
        ...
        requiem_list_for_each(&rule->destroy_context_list, tmp) {
                vcont = requiem_linked_object_get_object(tmp);
...
static int match_rule_list(pcre_plugin_t *plugin,
                           ...
                           const logeater_log_source_t *ls, logeater_log_entry_t *log_entry,
                           pcre_match_flags_t *match_flags)
{
        ...
        requiem_list_for_each(&rule->rule_list, tmp) {
                child = requiem_list_entry(tmp, pcre_rule_container_t, list);
        ...
        if ( state->idmef ) {
                *match_flags |= PCRE_MATCH_FLAGS_ALERT;
                ...
                if ( ! (rule->flags & PCRE_RULE_FLAGS_SILENT) ) {
                        requiem_log_debug(4, "logeater alert emit id=%d (last=%d) %s\n",
                         ...
                         */
                        logeater_alert_set_infos(state->idmef, log_entry);
...
int rule_regex_match(pcre_plugin_t *plugin, pcre_rule_container_t *rc,
                     const logeater_log_source_t *ls, logeater_log_entry_t *log_entry, pcre_match_flags_t *match_flags)
{
        int ret;
        pcre_state_t state;
...
static requiem_bool_t has_utf8(const char *regex)
{
        int support;
        unsigned char c;
...
rule_regex_t *rule_regex_new(const char *regex, requiem_bool_t optional)
{
        int err_offset;
        rule_regex_t *new;
        const char *err_ptr;
        ...
        if ( ! new->regex ) {
                requiem_log(REQUIEM_LOG_WARN, "unable to compile regex[offset:%d]: %s.\n", err_offset, err_ptr);
                free(new);
                return NULL;
        }
...
void rule_regex_destroy(rule_regex_t *ptr)
{
        ...
        requiem_linked_object_del((requiem_linked_object_t *) ptr);
        free(ptr);
}
